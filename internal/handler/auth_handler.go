...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{"code": 40001, "message": "请求参数错误"})
		return
	}
	...
	if strings.TrimSpace(req.Account) == "" || strings.TrimSpace(req.Password) == "" {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]interface{}{"code": 40002, "message": "账号和密码不能为空"})
		return
	}
	if len(req.Account) < 3 || len(req.Account) > 64 {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]interface{}{"code": 40003, "message": "账号长度必须在3-64个字符之间"})
		return
	}
	if len(req.Password) < 8 || len(req.Password) > 128 {
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]interface{}{"code": 40004, "message": "密码长度必须在8-128个字符之间"})
		return
	}
	...
	if locked {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{"code": 42901, "message": "操作过于频繁，请稍后再试"})
		return
	}
	...
	w.WriteHeader(result.StatusCode)
	json.NewEncoder(w).Encode(dto.NewLoginResponse(result.User))
}
...
func (h *AuthHandler) Logout(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{"code": 0, "message": "退出成功"})
}
